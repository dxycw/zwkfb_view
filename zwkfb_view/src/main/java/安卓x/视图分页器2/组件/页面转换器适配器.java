...
/**
 * Translates {@link 视图分页器2.OnPageChangeCallback} events to {@link 视图分页器2.PageTransformer} events.
 */
final class 页面转换器适配器 extends 视图分页器2.OnPageChangeCallback { //PageTransformerAdapter
    private final LinearLayoutManager mLayoutManager;

    private 视图分页器2.PageTransformer mPageTransformer;

    页面转换器适配器(LinearLayoutManager layoutManager) {
        mLayoutManager = layoutManager;
    }

    视图分页器2.PageTransformer getPageTransformer() {
     ...
     */
    void setPageTransformer(@Nullable 视图分页器2.PageTransformer transformer) {
