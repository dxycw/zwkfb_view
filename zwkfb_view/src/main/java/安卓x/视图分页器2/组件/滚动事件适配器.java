...
import static 安卓x.视图分页器2.组件.视图分页器2.ORIENTATION_HORIZONTAL;
import static 安卓x.视图分页器2.组件.视图分页器2.SCROLL_STATE_DRAGGING;
import static 安卓x.视图分页器2.组件.视图分页器2.SCROLL_STATE_IDLE;
import static 安卓x.视图分页器2.组件.视图分页器2.SCROLL_STATE_SETTLING;
...
/**
 * Translates {@link RecyclerView.OnScrollListener} events to {@link 视图分页器2.OnPageChangeCallback} events
 * for {@link 视图分页器2}. As part of this process, it keeps track of the current scroll position
 * relative to the pages and exposes this position via ({@link #getRelativeScrollPosition()}.
 */
final class 滚动事件适配器 extends RecyclerView.OnScrollListener { //ScrollEventAdapter
    /** @hide */
    @Retention(SOURCE)
    ...
    private 视图分页器2.OnPageChangeCallback mCallback;
    private final @NonNull 视图分页器2 mViewPager;
    ...
    private @视图分页器2.ScrollState int mScrollState;
    private 滚动事件适配器.ScrollEventValues mScrollValues;
    ...
    滚动事件适配器(@NonNull 视图分页器2 viewPager) {
        mViewPager = viewPager;
        mRecyclerView = mViewPager.mRecyclerView;
        //noinspection ConstantConditions
        mLayoutManager = (LinearLayoutManager) mRecyclerView.getLayoutManager();
        mScrollValues = new 滚动事件适配器.ScrollEventValues();
        resetState();
    }
    ...
    private void updateScrollEventValues() {
        滚动事件适配器.ScrollEventValues values = mScrollValues;
        ...
        if (values.mOffsetPx < 0) {
            ...
            if (new 动画布局改变检测器(mLayoutManager).mayHaveInterferingAnimations()) {
    ...
    void setOnPageChangeCallback(视图分页器2.OnPageChangeCallback callback) {
        mCallback = callback;
    }
     ...
     *         {@link 视图分页器2#endFakeDrag()} is called.
     */
     ...
     * the ViewPager is settling towards its {@link 视图分页器2#getCurrentItem() current item}, or
    ...
    private void dispatchStateChanged(@视图分页器2.ScrollState int state) {
