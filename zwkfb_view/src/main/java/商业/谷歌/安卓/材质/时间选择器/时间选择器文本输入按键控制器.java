 ...
 */
class 时间选择器文本输入按键控制器 implements TextView.OnEditorActionListener, View.OnKeyListener {

    private final 芯片文本输入组合视图 hourLayoutComboView;
    private final 芯片文本输入组合视图 minuteLayoutComboView;
    private final 时间模型 time;

    private boolean keyListenerRunning = false;

    时间选择器文本输入按键控制器(芯片文本输入组合视图 hourLayoutComboView, 芯片文本输入组合视图 minuteLayoutComboView, 时间模型 time) {
    ...
    private void moveSelection(@时间选择器控件.ActiveSelection int selection) {
