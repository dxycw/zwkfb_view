...
class 加载指示器动画代理 {
    ...
    @Nullable
    加载指示器可绘制对象 drawable;
    加载指示器绘制代理.IndicatorState indicatorState;

    public 加载指示器动画代理(@NonNull 加载指示器规范 specs) {
        this.specs = specs;
        indicatorState = new 加载指示器绘制代理.IndicatorState();
    }

    /** Registers the drawable associated to this delegate. */
    protected void registerDrawable(@NonNull 加载指示器可绘制对象 drawable) {
    ...
    private static final Property<加载指示器动画代理, Float> ANIMATION_FRACTION =
            new Property<加载指示器动画代理, Float>(Float.class, "animationFraction") {
                @Override
                public Float get(加载指示器动画代理 delegate) {
                    return delegate.getAnimationFraction();
                }

                @Override
                public void set(加载指示器动画代理 delegate, Float value) {
                    delegate.setAnimationFraction(value);
                }
            };

    private static final FloatPropertyCompat<加载指示器动画代理> MORPH_FACTOR =
            new FloatPropertyCompat<加载指示器动画代理>("morphFactor") {
                @Override
                public float getValue(加载指示器动画代理 delegate) {
                    return delegate.getMorphFactor();
                }

                @Override
                public void setValue(加载指示器动画代理 delegate, float value) {
