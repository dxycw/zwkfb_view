...
import static 商业.谷歌.安卓.材质.时间选择器.径向视图组.LEVEL_1;
import static 商业.谷歌.安卓.材质.时间选择器.径向视图组.LEVEL_2;
import static 商业.谷歌.安卓.材质.时间选择器.径向视图组.LEVEL_RADIUS_RATIO;
...
class 时钟指针视图 extends View {
    ...
    private final List<时钟指针视图.OnRotateListener> listeners = new ArrayList<>();
    ...
    private 时钟指针视图.OnActionUpListener onActionUpListener;
    ...
    public 时钟指针视图(Context context) {
        this(context, null);
    }

    public 时钟指针视图(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, R.attr.materialClockStyle);
    }

    @SuppressLint("RestrictedApi")
    public 时钟指针视图(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void setHandRotationInternal(
        ...
        for (时钟指针视图.OnRotateListener listener : listeners) {
    ...
    public void addOnRotateListener(时钟指针视图.OnRotateListener listener) {
        listeners.add(listener);
    }

    public void setOnActionUpListener(时钟指针视图.OnActionUpListener listener) {
