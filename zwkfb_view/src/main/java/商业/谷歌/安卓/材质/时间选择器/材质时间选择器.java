 ...
 * guidelines</a>.
 */
public final class 材质时间选择器 extends DialogFragment implements 时间选择器视图.OnDoubleTapListener {
    ...
    private 时间选择器视图 timePickerView;
    private ViewStub textInputStub;

    @Nullable
    private 时间选择器时钟呈现器 timePickerClockPresenter;
    @Nullable private 时间选择器文本输入呈现器 timePickerTextInputPresenter;
    @Nullable private 时间选择器呈现器 activePresenter;
    ...
    @NonNull
    private static 材质时间选择器 newInstance(@NonNull 构建器 options) {
        材质时间选择器 fragment = new 材质时间选择器();
    ...
    public final View onCreateView(
        ...
        okButton.setOnClickListener(
                v -> {
                    if (activePresenter instanceof 时间选择器文本输入呈现器) {
                        时间选择器文本输入呈现器 presenter = (时间选择器文本输入呈现器) activePresenter;
                    ...
                    for (View.OnClickListener listener : positiveButtonListeners) {
                        listener.onClick(v);
                    }
                    dismiss();
                });
        ...
        cancelButton.setOnClickListener(
                v -> {
                    for (View.OnClickListener listener : negativeButtonListeners) {
                        listener.onClick(v);
                    }
                    dismiss();
                });
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle bundle) {
        ...
        if (activePresenter instanceof 时间选择器文本输入呈现器) {
            view.postDelayed(
                    () -> {
                        if (activePresenter instanceof 时间选择器文本输入呈现器) {
                            ((时间选择器文本输入呈现器) activePresenter).resetChecked();
                        }
                    },
                    100);
        }
    ...
    public final void onCancel(@NonNull DialogInterface dialogInterface) {
        for (DialogInterface.OnCancelListener listener : cancelListeners) {
    ...
    private void updateInputMode(MaterialButton modeButton) {
        ...
        activePresenter = initializeOrRetrieveActivePresenterForMode(inputMode, timePickerView, textInputStub);
    ...
    private 时间选择器呈现器 initializeOrRetrieveActivePresenterForMode(
            int mode, @NonNull 时间选择器视图 timePickerView, @NonNull ViewStub textInputStub) {
        if (mode == INPUT_MODE_CLOCK) {
            timePickerClockPresenter =
                    timePickerClockPresenter == null
                            ? new 时间选择器时钟呈现器(timePickerView, time)
                            : timePickerClockPresenter;
        ...
        if (timePickerTextInputPresenter == null) {
            ...
            timePickerTextInputPresenter = new 时间选择器文本输入呈现器(textInputView, time);
        }
    ...
    @Nullable
    时间选择器时钟呈现器 getTimePickerClockPresenter() {
        return timePickerClockPresenter;
    }

    @VisibleForTesting
    void setActivePresenter(@Nullable 时间选择器呈现器 presenter) {
     ...
     * com.google.android.material.timepicker.MaterialTimePicker#addOnPositiveButtonClickListener(View.OnClickListener)}.
     */
    ...
    /**
     * Removes a listener previously added via {@link
     * com.google.android.material.timepicker.MaterialTimePicker#addOnNegativeButtonClickListener(View.OnClickListener)}.
     */
    ...
    /**
     * Removes a listener previously added via {@link
     * com.google.android.material.timepicker.MaterialTimePicker#addOnCancelListener(DialogInterface.OnCancelListener)}.
     */
    ...
    /**
     * Removes a listener previously added via {@link
     * com.google.android.material.timepicker.MaterialTimePicker#addOnDismissListener(DialogInterface.OnDismissListener)}.
     */
    ...
    /** Used to create {@link com.google.android.material.timepicker.MaterialTimePicker} instances. */
    public static final class 构建器 {

        private 时间模型 time = new 时间模型();
        ...
        public 构建器 setInputMode(@InputMode int inputMode) {
        ...
        @NonNull
        @CanIgnoreReturnValue
        public 构建器 setTimeFormat(@时间格式 int format) {
            int hour = time.hour;
            int minute = time.minute;
            time = new 时间模型(format);
            time.setMinute(minute);
            time.setHourOfDay(hour);
            return this;
        }
        ...
        @NonNull
        @CanIgnoreReturnValue
        public 构建器 setTitleText(@Nullable CharSequence charSequence) {
        ...
        /** Creates a {@link 材质时间选择器} with the provided options. */
        @NonNull
        public 材质时间选择器 build() {
            return 材质时间选择器.newInstance(this);
        }
    }
