...
/**
 * A class used to manipulate the {@link 标签布局.SlidingTabIndicator}'s indicator {@link Drawable} at any
 * point at or between tabs.
 *
 * <p>By default, this class will size the indicator according to {@link
 * 标签布局#isTabIndicatorFullWidth()} and linearly move the indicator between tabs.
 *
 * <p>Subclasses can override {@link #setIndicatorBoundsForTab(标签布局, View, Drawable)} and
 * {@link #updateIndicatorForOffset(标签布局, View, View, float, Drawable)} (TabLayout, View, View,
 ...
 * #calculateIndicatorWidthForTab(标签布局, View)} and {@link
 * #calculateTabViewContentBounds(标签布局.TabView, int)} to capture the bounds of the tab or tab's content.
 */
class 标签指示器插值器 {
    ...
    /**
     * A helper method that calculates the bounds of a {@link 标签布局.TabView}'s content.
     *
     ...
     * (if present).
     *
     * @param tabView {@link 标签布局.TabView} for which to calculate left and right content bounds.
    ...
    static RectF calculateTabViewContentBounds(
            @NonNull 标签布局.TabView tabView, @Dimension(unit = Dimension.DP) int minWidth) {
        int tabViewContentWidth = tabView.getContentWidth();
        int tabViewContentHeight = tabView.getContentHeight();
        @SuppressLint("RestrictedApi") int minWidthPx = (int) ViewUtils.dpToPx(tabView.getContext(), minWidth);
     ...
     * selected.
     *
     * <p>This method accounts for {@link 标签布局#isTabIndicatorFullWidth()}'s value. If true, the
     ...
     *
     * @param tabLayout The tab's parent {@link 标签布局}
     ...
     */
    static RectF calculateIndicatorWidthForTab(标签布局 tabLayout, @Nullable View tab) {
        if (tab == null) {
            return new RectF();
        }

        // If the indicator should fit to the tab's content, calculate the content's width
        if (!tabLayout.isTabIndicatorFullWidth() && tab instanceof 标签布局.TabView) {
            return calculateTabViewContentBounds((标签布局.TabView) tab, MIN_INDICATOR_WIDTH);
        }
     ...
     *
     * @param tabLayout The {@link 标签布局} parent of the tab and indicator being drawn.
     ...
     */
    void setIndicatorBoundsForTab(标签布局 tabLayout, View tab, @NonNull Drawable indicator) {
