 ...
 * TabLayoutMediator.
 */
public final class 标签布局协调器 {
    @NonNull private final 标签布局 tabLayout;
    @NonNull private final ViewPager2 viewPager;
    private final boolean autoRefresh;
    private final boolean smoothScroll;
    private final 标签布局协调器.TabConfigurationStrategy tabConfigurationStrategy;
    @Nullable private RecyclerView.Adapter<?> adapter;
    private boolean attached;

    @Nullable private 标签布局协调器.TabLayoutOnPageChangeCallback onPageChangeCallback;
    @Nullable private 标签布局.OnTabSelectedListener onTabSelectedListener;
    ...
    public interface TabConfigurationStrategy {
         ...
         * 标签布局.Tab#setText(CharSequence)}, but any form of styling can be applied.
         *
         * @param tab The Tab which should be configured to represent the title of the item at the given
         *     position in the data set.
         * @param position The position of the item within the adapter's data set.
         */
        void onConfigureTab(@NonNull 标签布局.Tab tab, int position);
    }

    public 标签布局协调器(
            @NonNull 标签布局 tabLayout,
            @NonNull ViewPager2 viewPager,
            @NonNull 标签布局协调器.TabConfigurationStrategy tabConfigurationStrategy) {
    ...
    public void attach() {
        ...
        onPageChangeCallback = new 标签布局协调器.TabLayoutOnPageChangeCallback(tabLayout);
        viewPager.registerOnPageChangeCallback(onPageChangeCallback);

        // Now we'll add a tab selected listener to set ViewPager's current item
        onTabSelectedListener = new 标签布局协调器.ViewPagerOnTabSelectedListener(viewPager, smoothScroll);
        ...
        if (autoRefresh) {
            ...
            pagerAdapterObserver = new 标签布局协调器.PagerAdapterObserver();
    ...
    /**
     * Returns whether the {@link 标签布局} and the {@link ViewPager2} are linked together.
     */
    ...
    private static class TabLayoutOnPageChangeCallback extends ViewPager2.OnPageChangeCallback {
        ...
        TabLayoutOnPageChangeCallback(标签布局 tabLayout) {
     ...
     */
    private static class ViewPagerOnTabSelectedListener implements 标签布局.OnTabSelectedListener {
