...
class 材质时间选择器 : DialogFragment(), 时间选择器视图.OnDoubleTapListener {
    ...
    private fun restoreState(bundle: Bundle?) {
        ...
        time = bundle.getParcelable<时间模型?>(TIME_MODEL_EXTRA)
        if (time == null) {
            time = 时间模型()
        }
    ...
    override fun onCreateView(
        ...
        val root = layoutInflater.inflate(
            zwkfb.view.R.layout.material_timepicker_dialog,
            viewGroup
        ) as ViewGroup
        timePickerView = root.findViewById<时间选择器视图?>(R.id.material_timepicker_view)
        timePickerView!!.setOnDoubleTapListener(this)
        textInputStub = root.findViewById<ViewStub?>(R.id.material_textinput_timepicker)
        modeButton = root.findViewById<MaterialButton>(R.id.material_timepicker_mode_button)
        ...
        cancelButton = root.findViewById<Button?>(R.id.material_timepicker_cancel_button)
