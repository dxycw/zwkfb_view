...
//                    }
    ...
    置显示自定义视图事件(object : 显示自定义视图事件{
        override fun 显示自定义视图(视图: View?, 回调: WebChromeClient.CustomViewCallback?) {
            ...
            状态栏沉浸式.初始沉浸式(上下文).透明栏().状态栏深色字体(!上下文.是否是深色模式)
                .导航栏深色图标(!上下文.是否是深色模式).隐藏栏(BarHide.FLAG_HIDE_BAR).初始化()
        }
    })
    置隐藏自定义视图事件(object : 隐藏自定义视图事件 {
        ...
        override fun 隐藏自定义视图() {
            ...
            状态栏沉浸式.初始沉浸式(上下文).透明栏().自动状态栏深色模式启用(!上下文.是否是深色模式)
                .自动导航栏深色模式启用(!上下文.是否是深色模式).隐藏栏(BarHide.FLAG_SHOW_BAR).初始化()
        }
    })
    ...
    // 设置User-Agent
    网页设置.setUserAgentString(if (系统类.是否为平板(上下文)) 网页配置.Windows.夸克UA else 网页配置.Android.夸克UA)
