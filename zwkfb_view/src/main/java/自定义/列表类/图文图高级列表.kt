...
class 图文图高级列表 @JvmOverloads constructor(
    val 上下文: Context,
    val attrs: AttributeSet?,
    val defStyleAttr: Int = 0,
) : 回收视图(上下文, attrs, defStyleAttr) {

    private var 适配器: 图文图高级列表适配器? = null

    init {
        this.适配器 = 图文图高级列表适配器(上下文)
        this.setAdapter(适配器)
        this.setLayoutManager(GridLayoutManager(上下文, 1)) //列表布局,垂直,必须写
    }

    fun 添加项目(图标: Drawable? = null, 标题: String?, 按钮: Drawable? = null) {
        适配器?.图标?.add(图标)
        适配器?.标题?.add(标题)
        适配器?.按钮?.add(按钮)
        适配器!!.刷新()
    ...
    fun 添加项目(图标: Int = -1, 标题: String?, 按钮: Int = -1) {
        适配器?.图标?.add(if (图标 == -1) null else 上下文.int转Drawable(图标))
        适配器?.标题?.add(标题)
        适配器?.按钮?.add(if (按钮 == -1) null else 上下文.int转Drawable(按钮))
        适配器!!.刷新()
    }
    ...
    private class 图文图高级列表适配器(val 上下文: Context) : Adapter<viewHolder>(){
        ...
        override fun onBindViewHolder(holder: viewHolder, position: Int) {
            ...
            if (项目图标 != null){
                holder.列表图标.visibility = View.VISIBLE
                ...
                holder.列表图标.visibility = View.GONE
            }
            ...
            if (项目按钮 != null){
                holder.列表按钮.visibility = View.VISIBLE
                holder.列表按钮.setImageDrawable(项目按钮)
                val 按钮布局参数 = LinearLayout.LayoutParams(holder.列表按钮.layoutParams)
                按钮布局参数.width = 上下文.dp转px(按钮宽度)
                按钮布局参数.height = 上下文.dp转px(按钮高度)
                按钮布局参数.setMargins(上下文.dp转px(15))
                ...
                holder.列表按钮.visibility = View.GONE
            }
    ...
    private class viewHolder(项目视图: ViewGroup) : ViewHolder(项目视图) {
        ...
        var 列表按钮 = ImageView(项目视图.context).apply {
            val 按钮布局参数 = LinearLayout.LayoutParams(
                项目视图.context.dp转px(25),
                项目视图.context.dp转px(25)
            )
            按钮布局参数.setMargins(项目视图.context.dp转px(15))
            layoutParams = 按钮布局参数
        }
